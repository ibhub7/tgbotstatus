...
# Clear existing handlers to prevent duplicate logs or conflicts
...
async def lifespan(app: FastAPI):
    ...
    restart_time = datetime.now(IST).strftime('%d/%m/%Y %H:%M:%S')
    try:
        await bot.send_message(
            Config.OWNER_ID, 
            f"🚀 <b>sʏsᴛᴇᴍ ᴏɴʟɪɴᴇ & sᴛᴀʙʟᴇ</b>\n"
            f"✅ <b>ʜᴇᴀʟᴛʜ ᴄʜᴇᴄᴋs:</b> ᴘᴀssᴇᴅ\n"
            f"✅ <b>ʟᴏɢɢɪɴɢ:</b> ᴀᴄᴛɪᴠᴇ (bot.log)\n"
            f"⏰ <b>sᴛᴀʀᴛᴇᴅ ᴀᴛ:</b> <code>{restart_time} IST</code>"
        )
    ...
    yield
    # 5. Clean Shutdown
    logger.info("Stopping Bot and shutting down...")
    await bot.stop()
